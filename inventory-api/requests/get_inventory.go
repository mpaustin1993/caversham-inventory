...
func GetInventory(connection *sql.DB, w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Item_Name, &item.Category, &item.Quantity, &item.Unit, &item.Location, &item.Expiration_Date, &item.Restock_Threshold, &item.Note); err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
